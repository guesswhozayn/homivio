...
const Input: React.FC<InputProps> = ({ onChange, value, name, placeholder }) => (
  <input
    onChange={onChange}
    value={value}
...
function Checkout() {
  ...
  const [input, setInput] = useState<CheckoutFormData>({
    name: '',
    email: '',
    street: '',
    city: '',
  ...
  const context = useContext(SiteContext) as CartContext;
