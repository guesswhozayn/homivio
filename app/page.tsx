...
export default async function Home() {
  ...
  const inventoryCategorized = inventory.reduce((acc: Category[], next: InventoryItem) => {
  ...
  return (
    <>
      ...
      <div className="w-full">
        ...
        flex lg:flex-row flex-col">
          ...
          <div className="flex flex-1 justify-center items-center relative">
              <Showcase
                imageSrc={displayInventory[2].image}
              />
      ...
      <div className="my-8 flex flex-col lg:flex-row justify-between">
        <DisplaySmall
          imageSrc={displayInventory[0].image}
          title={displayInventory[0].name}
          subtitle={displayInventory[0].categories[0]}
          link={`/product/${slugify(displayInventory[0].name)}`}
        />

        <DisplaySmall
          imageSrc={displayInventory[1].image}
          title={displayInventory[1].name}
          subtitle={displayInventory[1].categories[0]}
          link={`/product/${slugify(displayInventory[1].name)}`}
        />

        <DisplaySmall
          imageSrc={displayInventory[2].image}
          title={displayInventory[2].name}
          subtitle={displayInventory[2].categories[0]}
          link={`/product/${slugify(displayInventory[2].name)}`}
        />

        <DisplaySmall
          imageSrc={displayInventory[3].image}
          title={displayInventory[3].name}
          subtitle={displayInventory[3].categories[0]}
          link={`/product/${slugify(displayInventory[3].name)}`}
        />
      </div>
    </>
  );
