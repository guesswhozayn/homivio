...
type FormState = 'signUp' | 'confirmSignUp' | 'signIn' | 'signedIn';
...
export default function Admin() {
  ...
  const toggleFormState = (newFormState: FormState) => {
    setFormState(newFormState);
  };

  const signUp = async (form: AdminForm) => {
    const { username, email, password } = form;
    // sign up logic would go here
    setFormState('confirmSignUp');
  };

  const confirmSignUp = async (form: AdminForm) => {
    const { username, authcode } = form;
    // confirm sign up logic would go here
    setFormState('signIn');
  };

  const signIn = async (form: AdminForm) => {
