...
interface SignUpProps {
  ...
  toggleFormState: (state: 'signUp' | 'confirmSignUp' | 'signIn' | 'signedIn') => void;
}

function SignUp({ signUp, toggleFormState }: SignUpProps) {
        ...
        <div className="w-full max-w-144">
          <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => signUp(formData)} className="bg-primary hover:bg-black text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button">
                Sign Up
              </button>
              <a className="inline-block align-baseline font-bold text-sm" href="#" onClick={() => toggleFormState('signIn')}>
                Already signed up?
              </a>
            </div>
          </form>
