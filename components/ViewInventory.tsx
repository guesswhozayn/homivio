...
function ViewInventory() {
  ...
  const editItem = (item: InventoryItem, index: number) => {
  ...
  const saveItem = async (index: number) => {
    ...
    updatedInventory[index] = currentItem as InventoryItem;
  ...
  const deleteItem = async (index: number) => {
    const updatedInventory = [...inventory.slice(0, index), ...inventory.slice(index + 1)];
    setInventory(updatedInventory);
  };
            ...
            return (
              <div className="border-b py-10" key={item.id}>
                <div className="flex items-center">
                  ...
                  <div role="button" onClick={() => saveItem(index)} className="m-0 ml-10 text-gray-900 text-s cursor-pointer">
            ...
            <div className="border-b py-10" key={item.id}>
              <div className="flex items-center">
                ...
                <div className="flex items-center m-0 ml-10 text-gray-900 text-s cursor-pointer">
                  ...
                  <p role="button" onClick={() => editItem(item, index)} className="text-sm ml-10 m-0">Edit</p>
                </div>
              </div>
            </div>
          )
